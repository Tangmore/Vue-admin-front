    ...
    <div>
        <div class="max-menu-bar">
            <el-menu :default-active="active" class="el-menu-demo" mode="horizontal">
                <el-menu-item v-for="(item,index) in showMenu" :index="item.id.toString()" :key="index"
                 @click="handleClick(item.id)">{{item.menuName}}</el-menu-item>
                <el-dropdown @command='menuChange' style="margin-top: 24px;" v-if="hideMenu.length!=0">
                    <span class="el-dropdown-link">
                        <i class="el-icon-more-outline"></i>
                    </span>
                    <el-dropdown-menu slot="dropdown">
                        <el-dropdown-item v-for="(item,index) in hideMenu" :index="item.id" :key="index"
                         :command='item.id'>{{item.menuName}}</el-dropdown-item>
                    </el-dropdown-menu>
                </el-dropdown>
            </el-menu>
        </div>
